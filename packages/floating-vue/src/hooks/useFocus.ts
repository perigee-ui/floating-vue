 ...
 */
export function useFocus(context: FloatingRootContext, props: UseFocusProps = {}): () => ElementProps | undefined {
	const {
		open,
		onOpenChange,
		events,
	...
	function onFloatingOpenChange({ reason }: { reason: OpenChangeReason }) {
		if (reason === 'reference-press' || reason === 'escape-key') blockFocusRef = true
	}
	...
	const referenceProps: ElementProps['reference'] = {
		...
		onFocus(event) {
			...
			if (visibleOnly && isElement(target)) {
				try {
					...
					// instead.
					if (isSafariMac) {
						if (!keyboardModalityRef && !isTypeableElement(target)) return
		...
		onBlur(event) {
			...
			timeoutRef = window.setTimeout(() => {
				...
				if (!relatedTarget && activeEl === domReferenceVal) return

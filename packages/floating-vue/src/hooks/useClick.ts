...
export function useClick(context: FloatingRootContext, props: UseClickProps = {}): () => ElementProps | undefined {
	const {
		open,
		onOpenChange,
		dataRef,
	...
	const referenceProps: ElementProps['reference'] = {
		...
		...(eventOption === 'mousedown' && {
			onMousedown(event) {
				...
				if (ignoreMouse && pointerTypeRef && isMouseLikePointerType(pointerTypeRef, true)) return

				if (
					toggle &&
					toValue(open) &&
					(stickIfOpen && dataRef.openEvent ? dataRef.openEvent.type === 'mousedown' : true)
				) {
		...
		...(eventOption === 'click' && {
			onClick(event) {
				...
				if (
					toggle &&
					toValue(open) &&
					(stickIfOpen && dataRef.openEvent ? dataRef.openEvent.type === 'click' : true)
				)
					onOpenChange(false, event, 'click')
				else onOpenChange(true, event, 'click')
			},
		}),
		...(keyboardHandlers && {
			onKeydown(event) {
				...
				if (event.defaultPrevented || isButtonTarget(event)) return
				...
				if (toggle && toValue(open)) onOpenChange(false, event, 'click')
				else onOpenChange(true, event, 'click')
			},
			onKeyup(event) {
				if (event.defaultPrevented || isButtonTarget(event) || isTypeableElement(domReference.value)) return

				if (!didKeyDownRef) return

...
export function useRole(context: FloatingRootContext, props: UseRoleProps = {}): () => ElementProps | undefined {
	...
	const referenceProps = computed<ElementProps['reference']>(() => {
		...
		if (ariaRole === 'tooltip' || role === 'label') {
			return {
				[`aria-${role === 'label' ? 'labelledby' : 'describedby'}`]: openValue ? floatingId : undefined,
			}
		}

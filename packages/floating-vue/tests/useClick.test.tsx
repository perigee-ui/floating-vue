...
const App = defineComponent({
	...
	setup(props) {
		...
		const { getReferenceProps, getFloatingProps } = useInteractions([useClick(context, props.clickProps)])
		...
		return () => (
			<>
				...
				{open.value && (
					<div ref={(el: any) => refs.setFloating(el)} role="tooltip" {...getFloatingProps()}>
						tooltip
					</div>
				)}
			</>
...
describe('`stickIfOpen` prop', async () => {
	const App = defineComponent({
		...
		setup(props) {
			...
			return () => (
				<>
					<button {...getReferenceProps({ ref: refs.setReference })} data-testid="reference">
						reference
					</button>
					{open.value && (
...
describe('non-buttons', () => {
	it('adds Enter keydown', async () => {
		const screen = render(<App button={false} clickProps={clickProps} />)
	...
	it('typeable reference does not receive space key handler', async () => {
		const screen = render(<App typeable={true} clickProps={clickProps} />)
...
it('reason string', async () => {
	const App = defineComponent({
		setup() {
			...
			return () => (
				<>
					<button ref={(el: any) => refs.setReference(el)} {...getReferenceProps()} data-testid="reference" />
					{open.value && (
						<div ref={(el: any) => refs.setFloating(el)} role="tooltip" {...getFloatingProps()} />
					)}
				</>

...
import { useClick, useFloating, useInteractions, useRole } from '../src/index.ts'
...
const App = defineComponent({
	...
	setup(props) {
		...
		const { getReferenceProps, getFloatingProps } = useInteractions([useRole(context, props.roleProps)])
		...
		return () => (
			<>
				...
				{open.value && <div {...getFloatingProps({ ref: refs.setFloating })}>Floating</div>}
			</>
		)
	},
})
...
describe('dialog', () => {
	it('sets correct aria attributes based on the open state', async () => {
		...
		expect(button.getAttribute('aria-controls')).toBe(screen.getByRole('dialog').query()!.getAttribute('id'))
		expect(button.hasAttribute('aria-describedby')).toBe(false)
...
describe('menu', () => {
	it('sets correct aria attributes based on the open state', async () => {
		...
		expect(button.getAttribute('id')).toBe(screen.getByRole('menu').query()!.getAttribute('aria-labelledby'))
		expect(button.getAttribute('aria-controls')).toBe(screen.getByRole('menu').query()!.getAttribute('id'))
		expect(button.hasAttribute('aria-describedby')).toBe(false)
		expect(button.getAttribute('aria-expanded')).toBe('true')
...
describe('listbox', () => {
	it('sets correct aria attributes based on the open state', async () => {
		...
		expect(button.getAttribute('aria-controls')).toBe(screen.getByRole('listbox').query()!.getAttribute('id'))
		expect(button.hasAttribute('aria-describedby')).toBe(false)
		expect(button.getAttribute('aria-expanded')).toBe('true')
...
describe('select', () => {
	it('sets correct aria attributes based on the open state', async () => {
		...
		expect(screen.getByTestId('item-1').query()!.getAttribute('aria-selected')).toBe('false')
		expect(screen.getByTestId('item-2').query()!.getAttribute('aria-selected')).toBe('true')
...
describe('combobox', () => {
	it('sets correct aria attributes based on the open state', async () => {
		...
		expect(screen.getByTestId('item-1').query()!.getAttribute('aria-selected')).toBe(null)
		expect(screen.getByTestId('item-2').query()!.getAttribute('aria-selected')).toBe('true')
